...
from .metrics import print_evaluation_from_embeddings, get_roc_score
from scipy.sparse import coo_matrix
from .link_prediction_helpers import mask_test_edges
...
def get_input_generator(args):
    ...
    adj , val_edges, val_edges_false, test_edges, test_edges_false = mask_test_edges(adj, 0.7 * (1 - args.train_fraction) , 0.3* (1 - args.train_fraction))
    graphs_list = [nx.convert_matrix.from_scipy_sparse_matrix(adj, create_using = nx.DiGraph)]
    ...
    return graphs_list, features, val_edges, val_edges_false, test_edges, test_edges_false 
...
class Batch_Loader_link_classification(object):
    def __init__(self,args):
        init_object = get_input_generator(args)
        self.graph = init_object[0]
        self.graph, self.features, self.val_edges, self.val_edges_false, self.test_edges, self.test_edges_false = init_object
        self.args=args
    def __call__(self):
        train_idx = np.arange(self.features.shape[0])
...
def model_creation_util(parameterization,args, data_args):
    ...
    model_input_args = dict(feature_dim_size=args.feature_dim_size, ff_hidden_size=parameterization['ff_hidden_size'],
                                ...
                                num_U2GNN_layers=parameterization['num_hidden_layers'], device=args.device, sampler_type = args.sampler_type, loss_type = args.loss_type, adj_mat = None,single_layer_only = args.single_layer_only, ml_model_type = args.ml_model_type, projection_dim = args.projection_dim, features_in = features_in, graph_list = args.graph_obj)
...
def loss_func(args, logits):
    ...
    if( args.loss_type == "contrastive"):
        ...
        raise ValueError("loss type {} unsupported in link prediction".format(args.loss_type))
    return loss
...
def evaluate(epoch, data_args, model_args, args,validate = True):
    model = model_args.model
    ...
    with torch.no_grad():
        ...
        for fold_idx in range(2):
            if(args.eval_type=="lp"): 
                if(validate):
                    vt,vf = data_args.batch_nodes.get_validation()
                else:
                    print_str = "test"
...
def train_evaluate(data_args,model_args,args):
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if(epoch == args.num_epochs):
            validate = False
            print_str = "test"
        mean_10folds, std_10folds = evaluate(epoch, data_args, model_args, args, validate)
        print('| {} epoch {:3d} | time: {:5.2f}s | loss {:5.2f} | mean {:5.2f} | std {:5.2f} | '.format(print_str,
